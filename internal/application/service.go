...
type Service struct {
	...
	queryMu    sync.Mutex
	queryCache map[string]BatchListResult
}

func New(repo *storage.Repository) *Service {
	return &Service{repo: repo, clock: time.Now, queryCache: map[string]BatchListResult{}}
}
...
func (s *Service) QueryBatches(filter BatchListFilter) (BatchListResult, error) {
	cacheKey := fmt.Sprintf("%s|%s|%s|%s|%t|%d|%d", filter.BatchID, filter.TelescopeID, filter.TargetSource, filter.State, filter.TodoOnly, filter.Page, filter.PageSize)
	s.queryMu.Lock()
	...
	result := BatchListResult{Batches: items[start:end], StateCounts: counts, TodoTotal: todoTotal, Pagination: Pagination{Page: filter.Page, PageSize: filter.PageSize, TotalItems: total, TotalPages: pages}, Filter: BatchListFilterView{BatchID: filter.BatchID, TelescopeID: filter.TelescopeID, TargetSource: filter.TargetSource, State: filter.State, TodoOnly: filter.TodoOnly}}
	s.queryMu.Lock()
...
func (s *Service) invalidateQueryCache() {
	s.queryMu.Lock()
	s.queryCache = map[string]BatchListResult{}
	s.queryMu.Unlock()
}
