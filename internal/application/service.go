...
func (s *Service) mutate(batchID, operation string, meta RequestMeta, command any, fn mutation) (*CommandResult, error) {
	...
	return s.mutateLocked(batchID, operation, meta, command, fp, fn)
}

func (s *Service) mutateLocked(batchID, operation string, meta RequestMeta, command any, fp string, fn mutation) (*CommandResult, error) {
	batch, err := s.repo.Load(batchID)
	if err != nil {
		return nil, mapError(err)
	}
...
func (s *Service) RegisterSegment(batchID string, cmd RegisterSegmentCommand) (*CommandResult, error) {
	...
	unlockBatch := s.repo.Lock(batchID)
	defer unlockBatch()
	fp, err := fingerprint("segment.registered", cmd)
	if err != nil {
		return nil, err
	}
	record, err := s.repo.Idempotency(batchID, cmd.RequestID)
	if err != nil {
		return nil, err
	}
	if record != nil {
	...
	return s.mutateLocked(batchID, "segment.registered", cmd.RequestMeta, cmd, fp, func(b *domain.ObservationBatch) error { return b.AddSegment(cmd.Segment, s.clock()) })
}
...
func (s *Service) RegisterSegments(batchID string, cmd RegisterSegmentsCommand) (*CommandResult, error) {
	...
	for _, segment := range cmd.Segments {
		if _, _, err := s.repo.FindContentDigest(segment.ContentSHA256, batchID); err != nil {
			return nil, err
		}
	}
	unlock := s.repo.Lock(batchID)
