...
type Repository struct {
	...
	cache   map[string]*domain.ObservationBatch
...
func New(root string) (*Repository, error) {
	...
	repo := &Repository{root: root, cache: map[string]*domain.ObservationBatch{}, clock: time.Now}
...
func (r *Repository) loadSnapshot(id string) (*domain.ObservationBatch, error) {
	if err := validateStorageID(id); err != nil {
		return nil, err
	}
...
func (r *Repository) Commit(batch *domain.ObservationBatch, c Commit) error {
	...
	cachedBatch, err := cloneBatch(batch)
	if err != nil {
		return err
	}
	...
	r.cacheMu.Lock()
	if _, loaded := r.cache[batch.BatchID]; loaded {
		r.cache[batch.BatchID] = cachedBatch
	}
	r.cacheMu.Unlock()
	return nil
}

func cloneBatch(batch *domain.ObservationBatch) (*domain.ObservationBatch, error) {
	raw, err := json.Marshal(batch)
	if err != nil {
		return nil, err
	}
	var clone domain.ObservationBatch
...
func (r *Repository) appendEvent(id string, event audit.Event) error {
	f, err := os.OpenFile(r.eventPath(id), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o640)
	if err != nil {
		return err
	}
...
func (r *Repository) List() ([]*domain.ObservationBatch, error) {
	...
	for _, entry := range entries {
		...
		batch, err := r.loadSnapshot(entry.Name())
		if err != nil {
			return nil, err
		}
